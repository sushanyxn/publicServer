...
| **业务层** | 游戏玩法、场景、战斗、具体功能逻辑 | `slg-game/`、`slg-scene/`、`slg-shared-modules/`、`slg-web/`、`slg-log/`、`slg-robot/`、`slg-singlestart/` 等业务模块及其配置。**业务协议**（如登录、玩法相关 packet 及 message.yml 中对应注册）与**业务错误码**（如 LoginCode）即使位于 slg-common/slg-net 也归业务层 |
