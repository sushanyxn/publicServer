...
description: 为 slg-client 模拟客户端开发新的业务模块。根据用户指定的模块名称和对应的服务端协议，生成 ClientHandler（协议处理）、IClientModule 面板（UI 展示）和 HTTP API 端点。遵循客户端模拟器的架构约定和命名规范。在用户要求为模拟客户端新增模块、对接新协议、开发新面板或扩展客户端功能时使用。
---
...
- `slg-client/src/main/java/com/slg/client/http/ClientApiController.java` — HTTP API 示例
...
| HTTP API（可选） | `http/` | 扩展 REST 端点供 AI 调用 |
...
若需要 AI 通过 HTTP 操作该模块，在 `http/` 下新增 Controller：
...
public class HeroApiController {
    ...
    @GetMapping("/{accountName}/list")
    public Object heroList(@PathVariable String accountName) {
        ClientAccount account = accountManager.getByAccountName(accountName);
        // 返回模块数据
    }
}
```
