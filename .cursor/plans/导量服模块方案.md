...
`slg-web` 与 `slg-game`/`slg-scene` 同级（三级模块），依赖 `slg-net`（含 RPC + Zookeeper） + `slg-support`（含 MySQL/JPA） + `slg-redis`（含 RedisTemplate），**不依赖** `slg-fight`/`slg-game`/`slg-scene`。
