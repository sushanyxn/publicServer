...
在编写代码或测试前，可先查阅以下自动生成的速查文档快速了解协议与 GM 指令：

| 文档 | 路径 | 说明 |
|------|------|------|
| **协议总览** | `.cursor/protocol/协议总览.md` | 全部协议的速查表（协议号、类名、类型、模块、说明）与模块索引 |
| **协议模块文档** | `.cursor/protocol/{模块名}.md` | 各模块协议的详细信息（协议号、类型、说明、字段列表、继承关系） |
| **GM 指令总览** | `.cursor/gm/GM指令总览.md` | 全部 GM 指令的速查表（指令名、参数、模块、说明） |
| **GM 模块文档** | `.cursor/gm/{模块名}.md` | 各模块 GM 指令的详细用法与参数说明 |

- 协议文档可通过 **protocol-scan** skill 重新生成（当协议有增删改时）。
- GM 文档可通过 **gm-scan** skill 重新生成（当 GM 指令有增删改时）。
- 测试游戏功能时优先使用 **business-test** skill（MCP 工具实机测试），查阅 GM 指令和协议时先看上述速查文档。
