 ...
 */
@Component
public class PlayerGameRoute extends AbstractRpcRoute {
    /**
    ...
    public void sendMsg(IM_RpcRequest request, RpcMethodMeta rpcMethodMeta, Object... params){
        ...
        int gameServerId = routeSupportService.getPlayerGameServerId(playerId);
        NetSession netSession = routeSupportService.getSessionByServerId(gameServerId);

        // RPC 功能单一化，不负责连接的创建
        if (netSession == null || !netSession.isActive()) {
            LoggerUtil.error("[RPC] 未找到与服务器 {} 的有效连接", gameServerId);
            throw new RpcException("未找到与服务器的有效连接: " + gameServerId);
        }

        // 发送消息
        netSession.sendMessage(request);
        LoggerUtil.debug("[RPC] 发送消息到服务器: serverId={}, method={}",
                gameServerId, request.getMethodMarker());
    }
    ...
    public int resolveTargetServerId(RpcMethodMeta rpcMethodMeta, Object... routeParams) {
        ...
        return routeSupportService.getPlayerGameServerId(playerId);
    }
    ...
    public boolean isLocal(RpcMethodMeta rpcMethodMeta, Object... params){
        ...
        // 判断是否是本地服务器
        return routeSupportService.isLocal(gameServerId);
    }
}
